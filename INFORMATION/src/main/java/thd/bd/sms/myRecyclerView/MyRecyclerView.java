...
//        mRecyclerView.addItemDecoration(new SpaceItemDecoration(space));
    ...
    /**
     * //     * 当用户使用完下拉刷新回调时，需要调用此方法，将头部去隐藏，将STATE恢复
     * //
     */
...
//        toast.show();
     ...
     */
    public void loadMroeFinish(boolean isEnd) {
//        getRecyclerView().getAdapter().notifyDataSetChanged();
        STATE = DEFAULT;
        if (getRecyclerView().getAdapter() instanceof MainRecyclerAdapter) {
            ((MainRecyclerAdapter) getRecyclerView().getAdapter()).hideFootView(isEnd);
        }
